        ...
        if (assignee === undefined) {       //if assignee is undefined, group will also be undefined
            ...
            return app.promise(function(done, fail) { 
                app.ajax('getSingleAgent', assignee.id()).done(function(agent) { 
                    ...
                    var failMessage = "PREVENTED TICKET SAVE<br/>";
                    var warning = 'Warning: ' + agent.name + ' is unavailable, if this request requires immediate attention please re-assign to a different agent.';
