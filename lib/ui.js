...
module.exports = {
    ...
    renderStatusModal: function(agentID) { // review this
        util.appFramework.ajax('getSingleAgent', agentID).done(function(agent) {
            ...
            util.appFramework.trigger('functional_error', {type: 'renderStatusModal', agentID: agentID});
            util.appFramework.trigger('network_error', {request: 'getSingleAgent', type: 'ajax', agentID: agentID, error: error}));
        });
    },
    ...
    renderNavBar: function(filter) {
        ...
        util.getAll('users', ['getAllAgents', 1]).done(function(data) {
            ...
            util.appFramework.trigger('functional_error', {type: 'renderNavBar', filter: filter});
        });
    ...
    renderUser: function() {
        ...
        if (viewedUser.role() != 'end-user') {
            ...
            .done(function(user) {
                ...
                util.appFramework.trigger('functional_error', {type: 'renderUser', agentID: viewedUser.id()});
                util.appFramework.trigger('network_error', {request: 'getSingleAgent', type: 'ajax', agentID: viewedUser.id(), error: error}));
            });
        } else {
    ...
    renderTicket: function() { 
        ...
        if(asignee !== undefined) {
            ...
            .done(function(data){
                ...
                util.appFramework.trigger('functional_error', {type: 'renderTicket', agentID: viewedUser.id()});
                util.appFramework.trigger('network_error', {request: 'getSingleAgent', type: 'ajax', agentID: viewedUser.id(), error: error}));
            });
        } else {
            app.switchTo('ticket');
    ...
    renderSave: function(data) { 
        ...
        var failMessage = '<p style="margin-top: 16px; margin-bottom: 10px; text-align: center; font-weight: bold; font-size: 20px;">TICKET NOT SAVED</p>';
        var warning = '<p style="margin-top: 16px; margin-bottom: 10px; text-align: center; font-size: 14px;"><strong>' + agent.name + '</strong> is <strong>UNAVAILABLE</strong><br/></p><p style="margin-top: 12px; margin-bottom: 6px; text-align: center; font-size: 14px;">If this request requires immediate attention please re-assign to a different agent</p>';
        ...
        if (assignee === undefined) {       //if assignee is undefined, group will also be undefined
            ...
            app.ajax('getSingleAgent', assignee.id()).done(function(agent) {
                ...
                util.appFramework.trigger('functional_error', {type: 'renderSave', assignee: assignee.id(), mainPromise: false});
                util.appFramework.trigger('network_error', {request: 'getSingleAgent', type: 'ajax', agentID: assignee.id(), error: error}));
            );
            return true;
        } else {
            return app.promise(function(done, fail) { 
                app.ajax('getSingleAgent', assignee.id()).done(function(agent) { 
                    ...
                    if(!agent.user_fields.agent_ooo) { //if agent isn't ooo, allow it TODO: fix this to be dynamic
                        done();
                        ...
                        if (app.currentLocation() === 'new_ticket_sidebar') {  //can't assign a new ticket to an ooo user 
                            ...
                            app.ajax('getSingleTicket', ticket).done(function(ticket) {
                                if(ticket.ticket.assignee_id == assignee.id()) {            //check if ticket is still assigned to the same person
                                    ...
                                    fail(failMessage + warning);                            //otherwise, fail
                                }
                            }).fail(function(fail) {
                                util.appFramework.trigger('functional_error', {type: 'renderSave', ticket: ticket, mainPromise: true, request: 'getSingleTicket'});
                                util.appFramework.trigger('network_error', {request: 'getSingleTicket', type: 'ajax', ticket: ticket, error: error}));
                            });
                        }
                    }
                }).fail(function(fail) {
                    util.appFramework.trigger('functional_error', {type: 'renderSave', assignee: assignee.id(), mainPromise: true, request: 'getSingleAgent'});
                    util.appFramework.trigger('network_error', {request: 'getSingleAgent', type: 'ajax', agentID: assignee.id(), error: error}));
                });
            });
        }
