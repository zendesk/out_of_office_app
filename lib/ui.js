var requests = {
    ...
    getAllAgentsByGroup: function(page, group) {
        return {
            url: helpers.fmt('https://3sverige1423134029.zendesk.com/api/v2/groups/%@/users.json?role[]=agent&role[]=admin&page=%@', group, page)
        };
    },
...
module.exports = {
    ...
    renderNavBar: function(fresh) {
        ...
        var userListMode = 'getAllAgents';
        var filterGroup;        
        if (util.settings.assignableAgentsGroupId && !util.settings.userIsOOOAdmin) {
            userListMode = 'getAllAgentsByGroup';            
        }                
        util.getAll('users', [userListMode, 1, util.settings.assignableAgentsGroupId]).done(function(data) {            
            util.users = data;
