var requests = {
    ...
    getSingleGroup: function(group_id) {
        return {
            url: helpers.fmt('/api/v2/groups/%@/users.json?include=abilities', group_id)
        };
    }
};
...
var util = {
    ...
    getDefaultGroup: function(app) {
        app.ajax('getSingleAgent', app.currentUser().id()).done(function(user) {
            var agent = user.user;
            app.options.defaultGroupId = agent.default_group_id || 'group_0';
            util.selected_group = app.options.defaultGroupId;
            app.trigger("render_app");
        });
        console.log("getDefaultGroup");
    },

    filterByGroup: function(passedApp) {
        var groupID = util.appFramework.$('#filter_groups').val();
        var app = passedApp || util.appFramework; //util.appFramework gets overwritten if multiple render functions are called at the same time, keep a local copy
        if(groupID !== undefined) {
            util.selected_group = groupID;
        } else {
            groupID = util.selected_group;
        }
...
module.exports = {
    ...
    renderNavBar: function(fresh) {
        var app = util.appFramework;
        if(!app.options.defaultGroupId) {
            util.getDefaultGroup(app);
        } else if(util.groups === undefined) {
            util.groups = [];
            util.getAll('groups', ['getAllGroups', 1]).done(function(data) {
                util.groups = data;
                util.filterByGroup(app);
            }).fail(function() {
                app.trigger('functional_error', {location: 'renderNavBar'});
            });
        } else if(util.groups.length > 0) {
            util.filterByGroup(app);
        }
    },
