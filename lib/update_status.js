...
var util = {
    ...
    batchPost: function(request, data, page) {
        return util.appFramework.promise(function(done, fail) {
            console.log(data);
            if(page === undefined) {
                page = 1;    
            }
            util.appFramework.ajax(request, data[page]).done(function() {
                page++;
                if(data[page] === undefined) {
                    done();
                } else {
                    util.batchPost(request, data, page++);                
                }
            });
        ...
        return util.appFramework.promise(function(done, fail) {
            util.getView('rows', ['ticketPreview', agent.id]).done(function(tickets) { // changed by jeremiah
                ...
                if(tickets[0] === undefined) { 
                    ...
                    util.batchPost('unassignMany', util.splitArray(ticketIDs, 100)).done(function() {
