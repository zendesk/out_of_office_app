...
var util = {
    ...
    postPage: function(member, request, fn, data) {
        ...
        util.appFramework.ajax.apply(util.appFramework, request).done(function(newdata){ //get a page of the request
            ...
            if(newdata.next_page === null) { //check to see if there is another page to load
                ...
                util.postPage(member, request, fn, data); //if there is another page, call getAll again - will eventually reach the end and call done()
            }
        });
    },

    postAll: function(member, request) {
        return util.appFramework.promise(function(done, fail) {
            
            util.postPage(member, request, done);   
        });
    },
    ...
    unassignAll: function(agent) {
        return util.appFramework.promise(function(done, fail) {
            util.postAll('rows', ['ticketPreview', agent.id]).done(function(tickets) { // changed by jeremiah
