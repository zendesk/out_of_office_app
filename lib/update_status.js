var requests = {
    ...
    bulkTagTicket: function(paramaters) {
        ...
        data[paramaters.paramaters] = [this.options.userFieldKey];
        return {
            type: 'PUT',
            contentType: 'application/json',
            ...
            util.getView('rows', [ticketView, agent.id]).done(function(tickets){
                if(tickets[0] === undefined) {
                    ...
                    var isOut = agent.user_fields[util.settings.userFieldKey],
                        operator = (isOut) ? 'additional_tags'                     //depending on agent status, either tag or untag ticket
                                          : 'remove_tags',          
                    ...
                    util.appFramework.trigger('start_tagging', {agent: agent, count: ticketIDs.length});
                    util.bulkUpdate('bulkTagTicket', ticketIDs, operator, 5, function(complete) {
                        count = count + complete.idList.length;
                        var percentage = (count/tickets.length) * 100;
                        util.appFramework.updating[agent.id] = {tagging: true, percentage: percentage, user:agent};
                        util.appFramework.trigger('tag_progress', {agent: agent, total: tickets.length, count: count});
                        console.log(agent);
